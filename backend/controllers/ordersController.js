...
function getOwnerFilter(req) {
  return req.accountType === 'user'
    ? { userId: req.account._id }
    : { guestId: req.account._id };
}

const getAllOrders = async (req, res, next) => {
  try {
    const orders = await Order.find(getOwnerFilter(req))
      .sort({ orderTime: -1 });
...
const createOrder = async (req, res, next) => {
  try {
    ...
    if (req.accountType === 'guest') {
      orderData.expiresAt = req.account.expiresAt;
    }
